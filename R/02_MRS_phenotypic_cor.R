...
# twin 1####
phenotypes_t1 = twin_BMRQ_wide %>% select(music_discrimination_1,BAS_RR_1,BMRS_total_1) %>% rename("musical\nauditory\ndiscr." = music_discrimination_1, "general\nreward\nsensitivty" = BAS_RR_1,"music\nreward\nsensitivty" = BMRS_total_1)
observed_varCov_T1 =  cor(phenotypes_t1, use = "pairwise.complete.obs")

# compute standard deviations (Sd)
sd_mda_t1 = sd(phenotypes_t1$`musical\nauditory\ndiscr.`, na.rm = T)
...
observed_varCov_long_T1 = observed_varCov_long_T1 %>% 
  mutate(rowname = factor(rowname, levels = c("musical\nauditory\ndiscr.", "general\nreward\nsensitivty","music\nreward\nsensitivty")),
         colname = factor(colname, levels = rev(c("musical\nauditory\ndiscr.", "general\nreward\nsensitivty","music\nreward\nsensitivty"))))
...
p_crosstrait_1 = ggplot(observed_varCov_long_T1, aes(x = colname, y = rowname, fill = value)) +
  ...
  labs(x = NULL, y = NULL, subtitle = "",fill =  "cross\nphenotypic r")+
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  coord_equal(1)

# twin 2####
phenotypes_T2 = twin_BMRQ_wide %>% select(music_discrimination_2,BAS_RR_2,BMRS_total_2) %>% rename("musical\nauditory\ndiscr." = music_discrimination_2, "general\nreward\nsensitivty" = BAS_RR_2,"music\nreward\nsensitivty" = BMRS_total_2)
observed_varCov_T2 =  cor(phenotypes_T2, use = "pairwise.complete.obs")

# compute standard deviations (Sd)
sd_mda_T2 = sd(phenotypes_T2$`musical\nauditory\ndiscr.`, na.rm = T)
...
#SAVE####
save(p_crosstrait_1,file = sprintf("%s/%s/02_pheno_cor_T1.Rdata",wdOA,wdOA_output))
save(p_crosstrait_2,file = sprintf("%s/%s/02_pheno_cor_T2.Rdata",wdOA,wdOA_output))

pdf(sprintf("%s/%s/02_pheno_cor_T1.pdf",wdOA,wdOA_ImageOutput),
    width =4,
    height =3.5)
p_crosstrait_1
dev.off()

pdf(sprintf("%s/%s/02_pheno_cor_T2.pdf",wdOA,wdOA_ImageOutput),
    width =4,
    height =3.5)
p_crosstrait_2
dev.off()